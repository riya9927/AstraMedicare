...
const UpdateLabTechnician = ({ technician, onClose, onUpdateSuccess }) => {
    ...
    const [supervisedDoctors, setSupervisedDoctors] = useState([]);
    const [availableDoctors, setAvailableDoctors] = useState([]);

    // Initialize form data when labTechnician prop changes
    useEffect(() => {
        if (technician) {
            try {
                ...
                setFormData({
                    ...
                    addressLine1: technician.addressLine1 || '',
                    addressLine2: technician.addressLine2 || '',
                    qualifications: technician.qualifications || '',
                    shiftDetails: technician.shiftDetails || '',
                    salaryDetails: technician.salaryDetails?.toString() || ''
                });

                // Set supervised doctors
                if (technician.supervisedDoctors && Array.isArray(technician.supervisedDoctors)) {
                    setSupervisedDoctors(technician.supervisedDoctors.map(doc =>
                        typeof doc === 'object' ? doc._id : doc
                    ));
                }
                // Handle technician photo
                setPreviewImg(technician.photo || '');
            } catch (err) {
                console.error('Error initializing form data:', err);
                toast.error('Error loading lab technician data');
            }
        }
    }, [technician]);
    ...
    const handleDoctorChange = (e) => {
        ...
        if (isChecked) {
            setSupervisedDoctors([...supervisedDoctors, doctorId]);
        } else {
            setSupervisedDoctors(supervisedDoctors.filter(id => id !== doctorId));
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Append supervised doctors as array
            supervisedDoctors.forEach(doctorId => {
                formDataObj.append('supervisedDoctors[]', doctorId);
            });

            const { data } = await axios.put(
                `${backendUrl}/api/admin/lab/${technician._id}`,
                formDataObj,
                {
                    headers: {
                        aToken,
            ...
            if (data.success) {
                toast.success(data.message || 'Lab technician updated successfully');
                onUpdateSuccess();
                onClose();
            } else {
                toast.error(data.message || 'Failed to update lab technician');
            }
        } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 overflow-auto">
            <div className="bg-white rounded-xl p-8 w-full max-w-4xl shadow-lg animate-fade-in my-8 max-h-screen overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-bold text-blue-700">Edit Lab Technician</h3>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col md:flex-row items-center gap-6 mb-8">
                        <label htmlFor="technician-img-edit" className="cursor-pointer w-40 h-40 bg-blue-50 rounded-lg flex items-center justify-center overflow-hidden border border-blue-200 hover:border-blue-400 transition">
                            <img src={previewImg || assets.upload_area} alt="Lab Technician" className="w-full h-full object-cover" />
                        </label>
                        <input type="file" id="technician-img-edit" name="image" hidden onChange={handleFileChange} />
                        <p className="text-sm text-gray-600 text-center">
                            Click to update <br /> Lab Technician Photo
                        </p>
                    </div>
                    ...
                    <div className="mt-6">
                        <label className="block mb-3 font-medium">Supervised Doctors</label>
                                ...
                                availableDoctors.map(doctor => (
                                    <div key={doctor._id} className="flex items-center">
                                        <input
                                            ...
                                            value={doctor._id}
                                            checked={supervisedDoctors.includes(doctor._id)}
...
export default UpdateLabTechnician;